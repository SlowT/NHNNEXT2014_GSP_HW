...
bool ClientSession::PostConnect()
{
	...
	bind( mSocket, (SOCKADDR*)&mClientAddr, sizeof( mClientAddr ) );
	...
	if( FALSE == ConnectEx( mSocket, (SOCKADDR*)GIocpManager->GetServerAddr(), sizeof( SOCKADDR_IN ), NULL, 0, 0, (LPOVERLAPPED)connectContext ) )
	{
...
void ClientSession::ConnectCompletion()
{
	...
	{
		FastSpinlockGuard locked(mBufferLock);
		memset( mBuffer.GetBuffer(), (int)this, SEND_SIZE );
		mBuffer.Commit( SEND_SIZE );
	}
	if (false == PostSend())
	{
		printf_s("[DEBUG] PostSend error: %d\n", GetLastError());
	}
}
...
{
	FastSpinlockGuard criticalSection(mBufferLock);
	InterlockedAdd64( &GIocpManager->mRecvCount, transferred );
...
{
	FastSpinlockGuard criticalSection(mBufferLock);
	InterlockedAdd64( &GIocpManager->mSendCount, transferred );
