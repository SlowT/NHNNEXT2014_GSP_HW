...
IF EXISTS ( SELECT * FROM [dbo].[sysobjects] WHERE id = object_id(N'dbo.PlayerTable') )
...
BEGIN
	...
	SET NOCOUNT ON;
	UPDATE dbo.PlayerTable SET currentPosX=@posX, currentPosY=@posY, currentPosZ=@posZ WHERE playerUID=@playerUID
	SELECT @@ROWCOUNT
END
GO
...
BEGIN
	...
	SET NOCOUNT ON;
	SELECT playerName, currentPosX, currentPosY, currentPosZ, isValid, comment FROM dbo.PlayerTable WHERE playerUID=@playerUID
END		   
GO		   
