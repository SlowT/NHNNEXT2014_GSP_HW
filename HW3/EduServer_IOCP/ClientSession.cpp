...
bool ClientSession::PostAccept()
{
	...
	DWORD dwBytes;
	if( !GIocpManager->AcceptEx( *pListenSocket, mSocket, mBuffer.GetBuffer(), NULL, sizeof( sockaddr_in ) + 16, sizeof( sockaddr_in ) + 16, &dwBytes, (LPWSAOVERLAPPED)acceptContext ) ){
...
void ClientSession::DisconnectRequest(DisconnectReason dr)
{
	...
	GIocpManager->DisconnectEx( mSocket, &context->mOverlapped, NULL, NULL );
}
