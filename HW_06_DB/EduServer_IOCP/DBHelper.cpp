...
int DbHelper::mDbWorkerThreadCount = -1;
...
bool DbHelper::Initialize(const wchar_t* connInfoStr, int workerThreadCount)
{
	...
	for (int i = 0; i < mDbWorkerThreadCount; ++i)
	{
		...
		SQLRETURN ret = SQLDriverConnect( mSqlConnPool[i].mSqlHdbc, NULL, 
			const_cast<SQLWCHAR*>(connInfoStr), 
			SQL_NTS,
			NULL,
			0,
			NULL,
			SQL_DRIVER_NOPROMPT ); // =  SQLDriverConnect(...);
...
bool DbHelper::Execute(const wchar_t* sqlstmt)
{
	...
	SQLRETURN ret = SQLExecDirect( mCurrentSqlHstmt, const_cast<SQLWCHAR*>(sqlstmt), SQL_NTS ); //= SQLExecDirect(...);
	if (SQL_SUCCESS != ret && SQL_SUCCESS_WITH_INFO != ret)
	{
		PrintSqlStmtError();
		return false;
